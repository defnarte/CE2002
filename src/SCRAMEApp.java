...
public class SCRAMEApp {
	public static void main(String args[]) throws IOException {
		...
		ArrayList StudentAl = StudentDB.readStudents(filename);
		...
		while (choice >= 1 & choice <= 10) {
			switch (choice) {
				case 1:
					...
					StudentAl.add(stu);
					StudentDB.saveStudents("Students.txt",StudentAl);
					
					System.out.println("List of all students:");
					for (int i = 0; i < StudentAl.size(); i++) {
						Student stu2 = (Student) StudentAl.get(i);
				...
				case 3:
					...
					for (int i = 0; i < StudentAl.size(); i++) {
						Student stu3 = (Student) StudentAl.get(i);
